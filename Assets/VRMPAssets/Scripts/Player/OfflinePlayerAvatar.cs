...
namespace XRMultiplayer
{
    ...
    public class OfflinePlayerAvatar : MonoBehaviour
    {
        ...
        System.Action<bool> m_ConnectionChanged;
        ...
        void OnEnable()
        {
            ...
            XRINetworkGameManager.Connected.Subscribe(m_ConnectionChanged);
        }
        ...
        void OnDisable()
        {
            ...
            if (m_ConnectionChanged != null)
            {
                XRINetworkGameManager.Connected.Unsubscribe(m_ConnectionChanged);
                m_ConnectionChanged = null;
            }
        }
        ...
        void InitMic()
        {
            ...
            {
                Utils.Log("No microphone devices available. Offline avatar voice animation will remain disabled.", 1);
            ...
            m_MicInitialized = m_ClipRecord != null;
            s_Muted = !m_MicInitialized;
        }
        ...
        {
            m_MicInitialized = false;
            if (!string.IsNullOrEmpty(m_Device) && Permission.HasUserAuthorizedPermission(Permission.Microphone))
            {
        ...
        float LevelMax()
        {
            if (!m_MicInitialized || m_ClipRecord == null) return 0;
            float levelMax = 0;
            float[] waveData = new float[m_SampleWindow];
            int micPosition = Microphone.GetPosition(m_Device) - (m_SampleWindow + 1);
