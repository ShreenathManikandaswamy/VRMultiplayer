...
namespace XRMultiplayer
{
    ...
    public class XRINetworkGameManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Role selected for the next network join.
        /// </summary>
        public static XRPlayerRole LocalPlayerRole { get; private set; } = XRPlayerRole.Trainee;
...
# endif
        ...
        public virtual void QuickJoinLobbyAsTrainee()
        {
            SetLocalPlayerRole(XRPlayerRole.Trainee);
            QuickJoinLobby();
        }

        public virtual void QuickJoinLobbyAsSafetyOfficer()
        {
            SetLocalPlayerRole(XRPlayerRole.SafetyOfficer);
            QuickJoinLobby();
        }
        ...
        public virtual void SetLocalPlayerRole(XRPlayerRole role)
        {
            LocalPlayerRole = role;
            ApplySelectedPlayerRolePrefab();
        }

        void ApplySelectedPlayerRolePrefab()
        {
            if (NetworkManagerVRMultiplayer.Instance == null)
            {
                Utils.LogWarning($"{k_DebugPrepend}NetworkManagerVRMultiplayer not found. Cannot apply selected player role prefab.");
                return;
            }

            NetworkManagerVRMultiplayer.Instance.ApplyPlayerRolePrefab(LocalPlayerRole);
        }
